...
async function main() {
  ...
  const { tagName, downloadUrl, sha256Binary } = useLatest ? await fetchLatestMeta() : await fetchPinnedMeta();
  ...
  if (!cacheHit) {
    ...
    const binBuf = await downloadBinary(downloadUrl, sha256Binary);
    if (isPatch) {
