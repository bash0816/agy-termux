...
function getFreeDiskSpace(dirPath) {
  try {
    const result = execSync(`df -Pk "${dirPath}" | awk 'NR==2{print $4}'`, { shell: true, encoding: 'utf-8' });
...
async function extractAndVerify(tarballPath, tmpDir) {
  try {
  ...
  const binaryHash = await hashFile(extractedBinary);
...
async function downloadAndInstall() {
  ...
  try {
    ...
    const tarHash = await hashFile(tarballPath);
    ...
    extractedBinary = await extractAndVerify(tarballPath, tmpDir);
